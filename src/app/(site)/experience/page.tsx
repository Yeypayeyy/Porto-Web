...
function ExperienceCard({ item, index }: { item: Experience; index: number }) {
  return (
    <article
      className="reveal-card rounded-[8px] border border-white/10 bg-white/[0.055] p-5 shadow-[0_22px_58px_rgba(0,0,0,0.24)] backdrop-blur transition hover:-translate-y-1 hover:border-[#e2001a]/35 hover:bg-white/[0.075] md:p-6"
      style={{ animationDelay: `${index * 80}ms` }}
    >
      <div className="flex items-start justify-between gap-3">
        <span className="stitch-patch inline-flex rounded-[12px] border-2 border-dashed border-[#c9a24b] bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-[#4c4d4a] shadow-[0_10px_22px_rgba(0,0,0,0.18)]">
          {item.accent}
        </span>
        <span className="text-xs font-black text-[#e2001a]">
          0{index + 1}
        </span>
      </div>

      <div className="mt-5">
        <h3 className="text-2xl font-black leading-tight text-[#f7f5f2]">
          {item.organization}
        </h3>
        <p className="mt-2 text-sm font-black text-[#efece7]">{item.role}</p>
        <p className="mt-3 text-xs font-bold uppercase tracking-[0.12em] text-[#aeb5ae]">
          {item.period} / {item.location}
        </p>
      </div>

      <p className="mt-5 text-sm font-medium leading-7 text-[#d6d2cb] md:text-base md:leading-8">
        {item.description}
      </p>
    </article>
  );
...
export default function ExperiencePage() {
  return (
    <main className="min-h-screen bg-[#080b09] text-[#f7f5f2]">
      <Header />

      <section className="relative isolate overflow-hidden px-5 py-20 md:px-8 md:py-24">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_20%,rgba(226,0,26,0.16),transparent_24rem),radial-gradient(circle_at_82%_76%,rgba(226,0,26,0.14),transparent_24rem)]" />
        ...
        <div className="absolute bottom-0 left-0 z-10 h-6 w-full bg-[#9e0712]">
          <div className="project-stitch-line absolute inset-x-0 top-1/2 h-1 -translate-y-1/2" />
        </div>

        <div className="relative z-20 mx-auto max-w-6xl">
          <Link
            className="fun-link inline-flex text-sm font-black text-[#c9a24b]"
            href="/"
          >
            Back to home
          </Link>
          ...
          <div className="section-reveal mt-9 max-w-4xl">
            <p className="stitch-patch inline-flex w-fit rotate-[-2deg] rounded-[16px] border-2 border-dashed border-[#c9a24b] bg-white px-7 py-4 text-2xl font-black text-[#4c4d4a] shadow-[0_12px_28px_rgba(0,0,0,0.2)] md:px-8 md:py-5 md:text-3xl">
              Experience
            </p>
            <h1 className="section-title mt-6 text-5xl font-black leading-tight text-[#f7f5f2] md:text-7xl">
              Cerita peran, ruang bertumbuh, dan kerja bersama.
            </h1>
            <p className="section-copy mt-6 max-w-3xl text-lg font-medium leading-8 text-[#efece7] md:text-2xl md:leading-10">
              ...
              <section key={section.title} className="section-reveal-late">
                <div className="border-t border-white/10 pt-8">
                  <div>
                    ...
                    <h2 className="mt-3 text-4xl font-black leading-tight text-[#f7f5f2] md:text-5xl">
                      {section.title}
                    </h2>
                    <p className="mt-5 text-base font-medium leading-8 text-[#efece7]">
                      {section.copy}
                    </p>
                  </div>
                  ...
                  <div className="reveal-list mt-6 flex flex-wrap content-start gap-2">
                    {section.items.map((item) => (
                      <span
                        ...
                        className="rounded-full border border-white/12 bg-white/8 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-[#efece7]"
                      >
                        {item.accent}
                      </span>
                    ))}
                  ...
                  <figure className="relative mt-7 aspect-[16/10] overflow-hidden rounded-[8px] border border-white/10 bg-white/[0.055] p-1.5 shadow-[0_24px_70px_rgba(0,0,0,0.34)]">
                    ...
                    <figcaption className="absolute bottom-5 left-5 rounded-full border border-white/12 bg-[#080b09]/72 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-[#efece7] backdrop-blur">
                      {section.title}
                    </figcaption>
                  </figure>
                  ...
                  <div className="mt-5 grid gap-3">
                    {section.items.map((item) => (
                      ...
                      >
                        <p className="text-sm font-black text-[#f7f5f2]">
                          {item.organization}
                        </p>
                        <p className="mt-1 text-xs font-bold leading-5 text-[#d6d2cb]">
                          {item.role}
                        </p>
                      </div>
                    ))}
